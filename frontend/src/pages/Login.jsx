import { FileCheck2, LogIn, ShieldCheck, Zap } from "lucide-react";
        ...
        <div className="w-full max-w-sm py-10">
          ...
          >
            {busy ? (
              ...
              <LogIn size={17} strokeWidth={1.75} aria-hidden="true" />
            )}
            {/* This said "Continue with Google" behind a Google mark, which was
                true only while google was the one auth type configured. The
                modal now offers phone, X, Apple, Facebook, LinkedIn and GitHub
                as well, so naming one of them on the button promises the wrong
                thing to everyone who wanted another. */}
            {busy ? "Opening sign-in…" : "Sign in to continue"}
          </button>
          <p className="mt-2.5 text-center font-serif text-xs leading-relaxed text-ink-dim">
            Google, email, phone, X, Apple, Facebook, LinkedIn or GitHub — a wallet is created
            for you either way.
          </p>

          {/* The auth-methods line already survived the merge just above, so
              the copy from this side would only have duplicated it. The styling
              here is master's, which is the design system this page now uses. */}
