...
export default function CredentialsFooter({ onNavigate, onEnter }) {
  return (
    <footer className="border-t border-alabaster/10 bg-onyx text-alabaster">
      <div className="mx-auto max-w-[1180px] px-6 lg:px-14">
        {/* Team */}
        <div className="border-b border-alabaster/10 py-12 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="font-mono text-2xs uppercase tracking-macro text-teal-light">
                Who is behind this
              </p>
            ...
            <p className="max-w-[46ch] rounded-panel bg-state-pending/10 px-4 py-2.5 font-mono text-2xs uppercase leading-relaxed text-state-pending-light">
          ...
          <div className="grid gap-px overflow-hidden rounded-doc bg-alabaster/10 sm:grid-cols-2 lg:grid-cols-4">
            {TEAM.map((person) => (
              <div key={person.role} className="flex flex-col gap-3 bg-onyx p-6">
                <span
                  aria-hidden="true"
                  className="grid h-11 w-11 place-items-center rounded-full border border-dashed border-alabaster/35 font-mono text-sm text-alabaster/60"
                >
                  ?
                </span>
                <div>
                  <p className="font-mono text-sm text-alabaster/60">[ full name ]</p>
                  <p className="mt-1.5 text-[15px] font-medium text-alabaster">{person.role}</p>
                </div>
                <p className="mt-auto font-serif text-[14px] leading-relaxed text-alabaster/[0.68]">
                  {person.credential}
                </p>
              </div>
            ))}
          ...
          <div className="flex flex-wrap items-center gap-x-7 gap-y-2">
            ...
            {NAV.map((item) => (
              <button
                ...
                className="cursor-pointer text-sm text-alabaster/65 transition-colors duration-150 hover:text-alabaster"
              >
                {item.label}
              </button>
            ))}
            <button
              type="button"
              onClick={onEnter}
              className="flex cursor-pointer items-center gap-1 text-sm text-teal-light transition-colors duration-150 hover:text-alabaster"
            >
              Open workspace
