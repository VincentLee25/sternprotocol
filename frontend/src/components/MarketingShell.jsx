            ...
            {NAV.map((item) => (
              <button
                ...
                className={`cursor-pointer transition-colors duration-150 ${
                  current === item.id ? "text-teal" : "text-alabaster/60 hover:text-alabaster"
                }`}
              >
                {item.label}
              </button>
...
export function PageHeader({ eyebrow, title, lede }) {
  return (
    <header className="mx-auto max-w-[1180px] px-6 py-12 lg:px-14 lg:py-[76px]">
      <p className="font-mono text-2xs uppercase tracking-macro text-teal">{eyebrow}</p>
      <h1 className="mt-6 max-w-[18ch] text-balance text-[38px] font-medium leading-[1.04] tracking-display lg:text-[60px]">
        {title}
      </h1>
      <p className="mt-6 max-w-[62ch] font-serif text-[19px] font-light leading-[1.62] text-alabaster/[0.64]">
        {lede}
      </p>
    </header>
  );
        ...
        {eyebrow ? (
          <p
            className={`font-mono text-2xs uppercase tracking-macro ${
              tone === "teal" ? "text-white/60" : "text-alabaster/40"
            }`}
          >
            {eyebrow}
          </p>
        ...
        {intro ? (
          <p
            className={`mt-5 max-w-[64ch] font-serif text-[17px] font-light leading-[1.6] ${
              tone === "teal" ? "text-white/80" : "text-alabaster/[0.6]"
            }`}
          >
            {intro}
          </p>
